...
void erase_flash(void)
{
	...
	EraseInitStruct.NbSectors = PAGES;
	...
	{
		#if DEBUG
		 //CDC_Transmit_FS((unsigned char*)"Erase OK\n", 9);
...
uint32_t flash_search_adress(uint32_t address, uint16_t cnt)
{
	...
	while(count_byte)
	{
		if(0xFF == *(unsigned char*)address++) count_byte--;
		else count_byte = cnt;
		...
		if(address == ENDMEMORY - 1) // если достигнут конец флеша
		{
			...
			#if DEBUG
			//CDC_Transmit_FS((unsigned char*)"New cicle\n", 10);
...
void write_to_flash(myBuf_t *buff)
{
	...
	{
		#if DEBUG
		//CDC_Transmit_FS((unsigned char*)"Write buff ERROR\n", 17);
	...
	#if DEBUG
	//CDC_Transmit_FS((unsigned char*)"Write buff OK\n", 14);
...
void read_last_data_in_flash(myBuf_t *buff)
{
	...
	{
		#if DEBUG
		//CDC_Transmit_FS((unsigned char*)"Flash empty\n", 12);
...
int read_all_data_in_flash(myBuf_t *buff, uint8_t counter)
{
	if(res_addr == STARTADDR)
	{
		#if DEBUG
	...
	uint32_t adr_start = STARTADDR; // сдвигаемся на начало последних данных
	uint32_t adr;
  if(counter == 0)
	{
		for(uint16_t i = 0; i < BUFFSIZE; i++)
	  {
  		buff[i] = *(myBuf_t*)adr_start; // читаем
  		adr_start = adr_start + DATAWIDTH;
	  }
		adr = adr_start;
	}
	else
